...
这个仓库只包含流程代码和空白模板，不包含真实数据，也暂时不包含测试数据。
...
| `report` | 汇总 QC、图表、模块状态和文件引用 | 已完成的所选模块 |
...
报告只嵌入小型图表和摘要。H5AD、完整矩阵和原始图像保留在结果目录中，通过带大小和
...
显示证据不足。操作说明见[输入准备](docs/inputs.md#综合-qc-评分输入)。
