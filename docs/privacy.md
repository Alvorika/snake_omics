...
本仓库主要保存可复用流程，不包含私有项目数据或原始组学输入。作为公开 benchmark
例外，`tests/fixtures/libd_dlpfc_151673/` 保留了一个真实派生结果快照；其来源、删减
内容和路径替换均记录在 fixture README 中。
...
- 常规运行的 `work/`、`results/`、`logs/` 和 `.snakemake/`；
...
当前 LIBD 示例仅保留公开样本的派生结果；本机绝对路径已替换，大型 H5AD、原始矩阵和
Space Ranger 输入未进入仓库。
